...
async def create_campaign(
    ...
    if region_value:
        try:
            ...
            # Znajdź miasto po ID
            city = db.query(models.RegionCity).filter(models.RegionCity.id == city_uuid).first()
            if city:
    ...
    if category_id:
        ...
        if not category:
            raise HTTPException(status_code=400, detail="Kategoria nie została znaleziona")
        campaign_data['category_id'] = category_id
        campaign_data['category'] = category.name  # Zachowaj dla kompatybilności
    elif category_text:
        # Jeśli podano tekst kategorii, znajdź lub utwórz kategorię
        category = db.query(models.Category).filter(models.Category.name == category_text).first()
        if category:
            campaign_data['category_id'] = category.id
        else:
            # Jeśli kategoria nie istnieje, użyj tylko tekstu (kompatybilność wsteczna)
    ...
    if campaign.images:
        for idx, image_data in enumerate(campaign.images):
            db_image = models.CampaignImage(
                ...
                order_index=image_data.order_index if image_data.order_index is not None else idx,
...
async def update_campaign(
    ...
    db.query(models.CampaignImage).filter(models.CampaignImage.campaign_id == campaign_id).delete()
    db.query(models.CampaignRewardTier).filter(models.CampaignRewardTier.campaign_id == campaign_id).delete()
...
async def get_campaign_stats(campaign_id: UUID,
                            investments_status: Optional[schemas.InvestmentStatusEnum] = Query(None, description="Filter by investments status"),
                            db: Session = Depends(get_db),
                            current_user: models.User = Depends(utils.get_current_user)):
